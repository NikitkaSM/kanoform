...
const questionaryShow = () => {
	axios.get(`/api/questionary/${questionaryId}`)
		.then(response => {
			...
			const qualificationQuestionsContainer = document.getElementsByClassName("qualification-questions")[0];
			const featureQuestionsContainer = document.getElementsByClassName("feature-questions")[0];
			...
			if (questionary.qualification_questions.length !== 0) {
				questionary.qualification_questions.map(qQuestion => {
					...
					answerVariantsContainer.setAttribute("style", "display: flex; justify-content: center; flex-direction: column;")
					...
					qQuestion.answer_variants.map(answerVariant => {
						...
						div.setAttribute("style", "display: flex; margin-left: auto; margin-right: auto; flex-direction: column");
			...
			questionary.feature_questions.map(fQuestion => {
				...
				firstQuestion.innerText = "Насколько вам понравится наличие такой характеристики в продукте?";
				secondQuestion.innerText = 'А если эта характеристика будет отсутствовать или слабо выражена?';
				...
				thirdQuestionAnswerVariants.map(answerVariant => {
					...
					numberContainer.setAttribute("style",
			...
			const questionaryName = header.
				querySelector("h1")
				.innerText = questionary.name;
			const questionaryAuthor = header
				.querySelector("p")
